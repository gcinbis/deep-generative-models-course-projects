...
from calculate_aw import get_aw
import numpy as np

# Training routine for sngan_aw

def train_sngan_aw(batch_size=32, lr=0.1, cuda=True, num_epoch=100, num_disc_updates=1):
	...
	for epoch in range(num_epoch):
		...
		for iter_idx, data in enumerate(trainloader):
			...
			disc_real = F.relu(1 - real_scores).mean()
			disc_fake = F.relu(1 + fake_scores).mean()
			disc_loss = get_aw(disc_real, disc_fake, discriminator, optim_discriminator, real_scores, fake_scores)
		...
		if fid_score < best_fid:
			best_fid = fid_score
			torch.save(generator.state_dict, "best_sngan_aw_gen.ckpt")
			torch.save(discriminator.state_dict, "best_sngan_aw_disc.ckpt")
